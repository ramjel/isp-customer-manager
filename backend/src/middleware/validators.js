...
const customerValidation = [
  ...
  body('internet_plan').trim().notEmpty().withMessage('Internet plan is required'),
  body('status').isIn(['active', 'inactive']).withMessage('Status must be active or inactive'),
];

const planValidation = [
  body('name').trim().isLength({ min: 2, max: 100 }).withMessage('Plan name must be 2-100 characters'),
  body('speed_mbps').optional({ nullable: true }).isInt({ min: 1 }).withMessage('Speed must be a positive number'),
];
